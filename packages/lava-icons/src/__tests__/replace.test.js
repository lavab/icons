...
test('replaces [data-lava-icon] elements with SVG markup', () => {
  document.body.innerHTML =
    '<i data-lava-icon="icon1"></i><span data-lava-icon="icon2"></i>';
...
test('copies placeholder element attributes to <svg> tag', () => {
  document.body.innerHTML =
    '<i data-lava-icon="icon1" id="test" class="foo bar" stroke-width="1"></i>';
  expect(document.body.innerHTML).toMatchSnapshot();
  replace();
  expect(document.body.innerHTML).toMatchSnapshot();
});
